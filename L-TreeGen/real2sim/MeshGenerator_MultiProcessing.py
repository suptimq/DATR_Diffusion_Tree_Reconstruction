...
def process_json(file_info):
    ...
    if orchard_setup:
        if row == "new_tree_json":
            y_offset = row13_offset.value
            x_offset = 0
            row13_offset.value = row13_offset.value + 1
        elif row == "row15":
            x_offset = 3
            y_offset = row15_offset.value
            row15_offset.value = row15_offset.value + 1
        elif row == "row16":
            x_offset = 6
            y_offset = row16_offset.value
            row16_offset.value = row16_offset.value + 1
    ...
    data["trunk_radius"] = (
        taper_radius(
            data["trunk_points"], [data["trunk_radius"]], taper_angle=trunk_taper
        )
        if taper
    ...
    trunk_point_directions = calculate_direction_vectors(interpolated_trunk_points)
    ...
    for branch_idx, (branch, radii) in enumerate(
        ...
        taper_radii = (
            taper_radius(branch, radii, taper_angle=branch_taper) if taper else radii
        )
        ...
        l = branch_radius_stats["level1"]
        l.append(taper_radii[0])
        branch_radius_stats["level1"] = l
    ...
    for level in range(1, branch_level + 1):
        ...
        for branch_idx, (branch, radii) in enumerate(
            ...
            branch_file_name = f"{tree_id}_level{level+1}_branch{branch_idx+1}.lpy"
            with open(os.path.join(lpy_branch_dir, branch_file_name), "w") as file:
                file.write(branch_content)
            secondary_branch_counter.value += 1
            ...
            l = branch_radius_stats[f"level{level+1}"]
            l.append(taper_radii[0])
            branch_radius_stats[f"level{level+1}"] = l
...
if __name__ == "__main__":
    ...
    trunk_counter = manager.Value("i", 0)
    branch_counter = manager.Value("i", 0)
    secondary_branch_counter = manager.Value("i", 0)
    row13_offset = manager.Value("i", 0)
    row15_offset = manager.Value("i", 0)
    row16_offset = manager.Value("i", 0)
    trunk_json_file_list = manager.list()
    tree_skeleton_num_list = manager.list()
    # {'level1': [], 'level2': [], ...}
    branch_radius_stats = manager.dict()

    json_files = [x for x in os.listdir(data_folder) if x.endswith(".json")]
    ...
    # Convert Manager to regular Python for easier manipulation
    trunk_counter = trunk_counter.value
    branch_counter = branch_counter.value
    secondary_branch_counter = secondary_branch_counter.value
    row13_offset = row13_offset.value
    row15_offset = row15_offset.value
    row16_offset = row16_offset.value
    trunk_json_file_list = list(trunk_json_file_list)
    tree_skeleton_num_list = list(tree_skeleton_num_list)
    # TODO Empty??s
    branch_radius_stats = dict(branch_radius_stats)

    # Merge all trunk JSON files into one combined JSON file
    all_trunk_json_filepath = os.path.join(
        output_json_folder, "all_trunks_combined.json"
    )
    all_trunk_points = []
    all_trunk_radii = []
