...
class MVDiffusion(pl.LightningModule):
    def __init__(
        ...
        self.point_encoder = pipeline.point_encoder
    ...
    def prepare_batch_data(self, batch):
        ...
        cond_depths = batch['cond_depths']      # (B, C, H, W)
        cond_imgs = cond_imgs.to(self.device)
        cond_depths = cond_depths.to(self.device)
        ...
        cond_depths = v2.functional.resize(cond_depths, cond_size, interpolation=3, antialias=True).clamp(0, 1)
        ...
        return cond_imgs, cond_depths, cond_pcs, target_imgs
    ...
    def forward_vision_encoder(self, images):
        ...
        embeds = self.pipeline.vision_encoder(image_pt, output_hidden_states=False).image_embeds
        img_embeds, depth_embeds = embeds.chunk(2, dim=0)

        return img_embeds, depth_embeds

    def forward_point_encoder(self, points):
        dtype = next(self.point_encoder.parameters()).dtype
        points = points.to(device=self.device, dtype=dtype)

        embeds = self.point_encoder(points)
        global_embeds, _ = torch.max(embeds, dim=1, keepdim=False)

        return global_embeds

    @torch.no_grad()
    def forward_text_encoder(self, global_embeds):
        if global_embeds.ndim == 2:
    ...
    def training_step(self, batch, batch_idx):
        # get input
        cond_imgs, cond_depths, cond_pcs, target_imgs = self.prepare_batch_data(batch)
        ...
        if np.random.rand() < self.drop_cond_prob:
            ...
            cond_imgs_depths = torch.cat([cond_imgs, cond_depths])
            img_embeds, depth_embeds = self.forward_vision_encoder(cond_imgs_depths)
            point_embeds = self.forward_point_encoder(cond_pcs)
            global_embeds = img_embeds + depth_embeds + point_embeds
            prompt_embeds = self.forward_text_encoder(global_embeds)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        depths_pil = [v2.functional.to_pil_image(cond_depths[i]) for i in range(cond_depths.shape[0])]

        outputs = []
        for idx in range(len(images_pil)):
            cond_img = images_pil[idx]
            cond_depth = depths_pil[idx]
            cond_pc = cond_pcs[idx]
            latent = self.pipeline(cond_img, cond_depth, cond_pc, num_inference_steps=75, output_type='latent').images
