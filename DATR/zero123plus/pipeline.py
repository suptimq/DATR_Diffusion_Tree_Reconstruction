...
from zero123plus.models.point import TransformerPointTokenizer
...
class Zero123PlusPipeline(diffusers.StableDiffusionPipeline):
    ...
    point_encoder: TransformerPointTokenizer
    ...
    def __init__(
        ...
        point_encoder: Optional[TransformerPointTokenizer] = None,
    ...
    def __call__(
        ...
        **kwargs
        ...
        depth = to_rgb_image(depth)
        depth = self.feature_extractor_clip(images=depth, return_tensors="pt").pixel_values
        ...
        depth = depth.to(device=self.vae.device, dtype=self.vae.dtype)
        points = points.to(device=self.vae.device, dtype=self.vae.dtype).unsqueeze(0)
        ...
        # Image and colored depth embedding
        img_depth = torch.cat([image_2, depth])
        embeds = self.vision_encoder(img_depth, output_hidden_states=False).image_embeds
        img_embeds, depth_embeds = embeds.chunk(2, dim=0)

        # Point embedding
        point_embeds = self.point_encoder(points)
        point_embeds, _ = torch.max(point_embeds, dim=1, keepdim=False)
