...
class MVRecon(pl.LightningModule):
    def __init__(
        ...
        if lrm_path is not None:
            state_dict = torch.load(lrm_path, map_location='cpu')['state_dict']
            state_dict = {k[14:]: v for k, v in state_dict.items() if k.startswith('lrm_generator.')}
            self.lrm_generator.load_state_dict(state_dict, strict=True)
            print(f"Loaded LRM generator from {lrm_path}")
